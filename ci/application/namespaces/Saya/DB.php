...
Class DB extends Capsule {
	...
	public static function koneksi( $orm = FALSE )
	{
		...
		$koneksi=null;
		if( file_exists( APPPATH .'config/config_db.php' ) && self::$setting == array() ){
			//require (APPPATH .'config/config_db.php');
			//$koneksi = isset($db['laravel_orm']) ? $db['laravel_orm'] : null;
			$koneksi = \get_instance()->config->item('laravel_orm');
		}else{
			$koneksi = self::$setting ;
		}
		if(empty($koneksi))show_error('
			<strong style="font-weight:bold;">Database Connection Not Found <span style="color:red;">[ application/config/config_db.php ]</span> !!!</strong>
